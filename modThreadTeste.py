...
def acess_pokemons(pokemons):
    ...
    [dowload_file(name, url) for name, url in image_url.items()]
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=count_cores) as execute:
       execute.submit(acess_pokemons, pokemons)
